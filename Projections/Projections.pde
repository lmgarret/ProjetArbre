...
void draw() {
  ...
  My3DBox input3DBox = new My3DBox(origin, (int)(100*scaleValue), (int)(150*scaleValue), (int)(300*scaleValue));
  
  float[][] globalRotationX = rotateXMatrix(rotationX);  
  float[][] globalRotationY = rotateYMatrix(rotationY);
  
  //rotated around x
  float[][] transform1 = rotateXMatrix(PI/8);
  input3DBox = transformBox(transformBox(input3DBox, transform1), globalRotationX);
  input3DBox = transformBox(input3DBox, globalRotationY);

  projectBox(eye, input3DBox).render();
  //rotated and translated
  float[][] transform2 = translationMatrix(200, 200, 0);
  input3DBox = transformBox(transformBox(input3DBox, transform2), globalRotationX);
  input3DBox = transformBox(input3DBox, globalRotationY);
  projectBox(eye, input3DBox).render();
  //rotated, translated, and scaled
  float[][] transform3 = scaleMatrix(2, 2, 2);
  input3DBox = transformBox(transformBox(input3DBox, transform3), globalRotationX);
  input3DBox = transformBox(input3DBox, globalRotationY);
  projectBox(eye, input3DBox).render();
}
void mouseDragged() 
{
  ...
  scaleValue = (double)oldMouseY/mouseY;
}
void mouseReleased(){
   oldMouseY = -1;
}
